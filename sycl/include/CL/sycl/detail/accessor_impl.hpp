...
using AccessorImplPtr = detail::shared_ptr<AccessorImplHost>;
...
public:
  AccessorBaseHost(id<3> Offset, range<3> AccessRange, range<3> MemoryRange,
    ...
    impl = detail::shared_ptr<AccessorImplHost>(new AccessorImplHost(
...
using LocalAccessorImplPtr = detail::shared_ptr<LocalAccessorImplHost>;

class LocalAccessorBaseHost {
public:
  LocalAccessorBaseHost(sycl::range<3> Size, int Dims, int ElemSize) {
    impl = detail::shared_ptr<LocalAccessorImplHost>(
...
protected:
  ...
  detail::shared_ptr<LocalAccessorImplHost> impl;
};
