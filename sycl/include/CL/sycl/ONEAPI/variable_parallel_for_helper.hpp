//==----- variable_parallel_for_helper.hpp - SYCL data streams ---*- C++ -*-==//
//
...
/// Predicate returning true if all template type parameter is a data stream or
/// a reduction.
template <typename T> struct IsValidParam {
    static constexpr bool value =
        IsReduction<T>::value || IsDataStream<T>::value;
};

/// Predicate returning true if all template type parameters except the last one
/// are reduction or a data stream.
template <typename FirstT, typename... RestT> struct AreAllButLastValidParam {
  static constexpr bool value =
      IsValidParam<FirstT>::value &&
      AreAllButLastValidParam<RestT...>::value;
};

/// Helper specialization of AreAllButLastValidParam for one element only.
/// Returns true if the template parameter is not a reduction or a data stream.
...
template <typename Tuple, typename Seq>
using make_data_stream_index_sequence
  = make_filtered_index_sequence<IsDataStream, Tuple, Seq>;
...
struct IsReadableDataStreamPredicate {
  template <typename T> struct Func {
    static constexpr bool value =
...
struct IsWriteableDataStreamPredicate {
  template <typename T> struct Func {
    static constexpr bool value =
...
template <int Dims, typename ReadToTupleT, typename DataStreamAccessorTupleT>
void readStreamValueToTuple(id<Dims>, ReadToTupleT &,
                            DataStreamAccessorTupleT &,
                            std::index_sequence<>) {}

template <int Dims, typename ReadToTupleT, typename DataStreamAccessorTupleT,
          size_t I, size_t... Is>
void readStreamValueToTuple(id<Dims> gid, ReadToTupleT &vals,
                            DataStreamAccessorTupleT &streamAccs,
                            std::index_sequence<I, Is...>) {
  std::get<I>(vals) = std::get<I>(streamAccs)[gid];
  readStreamValueToTuple(gid, vals, streamAccs, std::index_sequence<Is...>{});
}
  
template <int Dims, typename DataStreamAccessorTupleT, typename ReadToTupleT>
void writeTupleValueToStream(id<Dims>, ReadToTupleT &,
                             DataStreamAccessorTupleT &,
                             std::index_sequence<>) {}

template <int Dims, typename DataStreamAccessorTupleT, typename ReadToTupleT,
          size_t I, size_t... Is>
void writeTupleValueToStream(id<Dims> gid, DataStreamAccessorTupleT &streamAccs,
                             ReadToTupleT &vals, std::index_sequence<I, Is...>){
